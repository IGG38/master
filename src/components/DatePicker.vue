<template>
  <div class="calendar">
    ...
    <div class="calendar-days" @dbclick="onPressDoubleClick(day)">
      ...
      <!-- 本月的日期 -->
      <span v-for="day in daysInMonth" :key="day" :class="['calendar-day', { today: isToday(day), selected: day === selectedDay, weekend: isWeekend(day), star: isTargetDate(day) }]" @click="selectDay(day)">
        {{ day }}
      </span>
      <!-- 下个月的日期（灰色显示） -->
...
export default {
  ...
  setup(props, { emit }) {
    ...
    const isToday = (day) => {
      const today = moment();
      return today.date() === day && today.month() === currentDate.value.month() && today.year() === currentDate.value.year();
    };
    ...
    const isTargetDate = (day) => {
      ...
      return moment(currentDate.value).date(day).isSame(moment(targetDate), 'day');
    };

    const selectDay = (day) => {
      const selectedDate = moment(currentDate.value).date(day);
      emit('selectDate', selectedDate.valueOf());
